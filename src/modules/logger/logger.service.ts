...
export class LoggerService {
  ...
  async logException(
    ...
    const metaData = {
      ...
      referer: request.headers['origin'] ?? request.headers['referer'],
    ...
    await this.logError({
      ref_website: request.headers['origin'] ?? request.headers['referer'],
  ...
  private async saveLog(level: LogLevel, params: LogParams) {
    const logEntry = this.loggerRepository.create({
      level,
      ref_website: params.ref_website,
